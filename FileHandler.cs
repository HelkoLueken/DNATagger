...
namespace DNATagger
{
    class FileHandler
    {
        ...
        public static List<DNASequenceAlt> readFasta(String path) {
            List<DNASequenceAlt> output = new List<DNASequenceAlt>();
            if (openFile(path))
            {
                ...
                else
                {
                    while (!fileReader.EndOfStream)
                    {
                        ...
                        if (line.ToCharArray()[0] == '>')
                        {
                            ...
                            {
                                output.Add(new DNASequenceAlt(fastaBlock.ToString(), src : path));
                                fastaBlock.Clear();
                            }
