...
namespace DNATagger
{
    class SequenceTrackAlt
    {
        private List<DNASequenceAlt> sequences = new List<DNASequenceAlt>();
        ...
        public SequenceTrackAlt(List<DNASequenceAlt> seqs) {
            foreach(DNASequenceAlt seq in seqs){
                addSequence(seq);
            }
            this.header = seqs.ElementAt(0).getSource();
        }

        public SequenceTrackAlt(DNASequenceAlt seq) {
            addSequence(seq);
        ...
        public void addSequence(DNASequenceAlt seq){
        ...
        public void draw(System.Windows.Forms.Panel canvas, Font font, bool showLetters) {
            ...
            foreach (DNASequenceAlt seq in sequences) seq.draw(canvas, font, showLetters);
        ...
        public DNASequenceAlt getSequence(int i) {
            return this.sequences.ElementAt(i);
        }



        public List<DNASequenceAlt> getSequences() {
        ...
        public int getLength() {
            int len = 0;
            foreach (DNASequenceAlt seq in sequences) {
